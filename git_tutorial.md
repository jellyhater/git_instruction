...
* __git log --graph --all__ - просмотр логов в графическом виде
...
для того, чтобы привязать локальный репозиторий необходимо выбрать на Github "new", затем ввести имя идентичное локальной папке, после чего ввести следующие команды:
```
git branch -M main
git remote add origin <ссылка на репозиторий Github.com>
